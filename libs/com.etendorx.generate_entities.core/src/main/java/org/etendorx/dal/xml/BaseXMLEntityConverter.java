...
public class BaseXMLEntityConverter implements OBNotSingleton {
  ...
  protected void addToInsertOrUpdateLists(String id, BaseOBObject bob, boolean writable,
    ...
    if (!writable && updated) {
      if (bob.isNewOBObject()) {
        warn(
            "Not allowed to create entity: " + bob.getEntityName() + " (import id: " + id + ") " + " because it is not writable");
        return;
      } else {
      ...
      if (hasReferenceAttribute && !bob.isNewOBObject()) {
        log.debug(
            "Entity {} ({}) has not been updated because it already exists and it is imported as a reference from another object",
            bob, bob.getEntity().getTableName());
      }
      if (!writable && !bob.isNewOBObject() && !hasReferenceAttribute) {
        warn(
            "Not updating entity: " + bob.getIdentifier() + "(" + bob.getEntityName() + ") " + " because it is not writable for this user");
      }
  ...
  protected BaseOBObject replaceByUniqueObject(BaseOBObject bob) {
    ...
    if (bob.isNewOBObject() && bob.getEntity().getUniqueConstraints().size() > 0) {
      if (bob.getEntity().getTableName().toLowerCase().endsWith("_trl") || ("".equals(
          bob.getId()) || bob.getId() == null)) {
    ...
    for (BaseOBObject bob : list) {
      for (Property p : bob.getEntity().getProperties()) {
        ...
        if (p.isOneToMany() && p.isChild()) {
          @SuppressWarnings("unchecked") final List<Object> childList = (List<Object>) bob.get(
              p.getName());
          int index = -1;
          while ((index = childList.indexOf(prevObject)) != -1) {
            Check.isTrue(childList.get(index) == prevObject,
                "Object" + prevObject + " not present on index " + index + " of property " + p + " of object " + bob);
...
*/
   ...
   * objects should be retrieved from the database.
   *
   * @param optionClientImport sets the value of the client import option (default is false)
   */
