...
import org.etendorx.dal.security.EntityAccessChecker;
...
public class OBContext implements OBNotSingleton, Serializable {
   ...
   *
   * @param doOrgClientAccessCheck Whether entity access (client+org) should also be checked
  ...
  private static void printUnbalancedWarning(boolean printLocationOfCaller, AdminType type) {
    if (ADMIN_TRACE_SIZE == 0) {
      String errMsg = "Unbalanced calls to " + type.setMethod + " and " + type.restoreMethod + ". " + "Consider setting the constant OBContext.ADMIN_TRACE_SIZE to a value higher than 0 to debug this situation";
   ...
   *
   * @param request the http request used to get the http session
   * @param context the context which will be stored in the session
   */
   ...
   *
   * @param userId the id of the user (as present in the database)
   */
   ...
   * @param roleId   the id of the role under which the user is currently working
   * @param clientId the id of the user's client
   * @param orgId    the ud of the user's organization
   */
   ...
   * @param clientId     the id of the user's client
   * @param orgId        the ud of the user's organization
   * @param languageCode the selected language, if null then the user language is read.
   */
  public static void setOBContext(String userId, String roleId, String clientId, String orgId,
      String languageCode) {
   ...
   * @param orgId        the ud of the user's organization
   * @param languageCode the selected language, if null then the user language is read.
   * @param warehouseId  the id of the current warehouse of the user.
   */
  public static void setOBContext(String userId, String roleId, String clientId, String orgId,
   ...
   *
   * @param userId the user used for creating the context
   ...
   * internals.
   *
   * @param obContext the context to set in the thread
   */
   ...
   *
   * @param role the role used to initialize the readable clients
   */
  /*
...
*/
   ...
   *
   * @param orgId the id of the additional writable organization
   */
   ...
   *
   * @param request the http request used to set the OBContext
      ...
      if (roleId != null) {
        ...
        if (ur == null) {
          throw new OBSecurityException(
              "Your user is not assigned to a Role and it is required to login into etendorx. Ask the Security Administrator");
        }
...
*/
  ...
  */
  ...
  public String toString() {
    return "[user: " + userID + ", role:" + roleID + ", client:" + clientID + ", org:" + orgID + ", warehouse: " + warehouseID + ", lang:" + langID + "]";
  }
