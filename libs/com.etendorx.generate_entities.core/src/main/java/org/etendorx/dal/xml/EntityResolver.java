...
import org.etendorx.dal.service.OBCriteria;
...
/*
import static org.etendorx.model.ad.system.Client.PROPERTY_ORGANIZATION;
import static org.etendorx.model.common.enterprise.Organization.PROPERTY_CLIENT;
*/
...
public class EntityResolver implements OBNotSingleton {
   ...
   * @param referenced if the entity is searched because it is refered to or if it is in the root of the xml
  ...
  public BaseOBObject resolve(String entityName, String id, boolean referenced,
    ...
    if (result == null && id != null) {
      //result = doSearch(id, entity, null /*client.getId()*/, filterOrgs, "");
    }
...
*/
  ...
  protected BaseOBObject doSearch(String id, Entity entity, String orgId) {
    boolean filterOrgs = true;
    return doSearch(id, entity, filterOrgs, orgId);
  }

  protected BaseOBObject doSearch(String id, Entity entity, boolean filterOrgs, String orgId) {
  ...
  private List<RefDataLoaded> getRefLoadedsUsingSql(String id, Entity entity, String orgId,
    ...
    try {
      ...
      List<RefDataLoaded> refDataLoadeds = new ArrayList<RefDataLoaded>();
      while (rs.next()) {
...
*/
   ...
   *
   * @param optionCreateReferencedIfNotFound if true then referenced objects are allowed to not exist in the database, meaning that
...
*/
   ...
   *
   * @param prevObject the object current in the data cache
   * @param newObject  the new object which can then be found under the old object
   */
