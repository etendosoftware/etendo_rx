...
/**
 * Producer used only with testing porpouses
 */
@Slf4j
public class AsyncProcessDbProducer {
  ...
  public static void main(String[] args) {
    KafkaProducer<String, String> asyncProcessExecutionProducer = new KafkaProducer<>(
        Map.of(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:29092",
            ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class,
            ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, StringSerializer.class));

    List<AsyncProcessExecution> data1 = List.of(AsyncProcessExecution.builder()
        ...
        .time(new Date())
        .build());
    List<AsyncProcessState> stateList = List.of(AsyncProcessState.ACCEPTED,
        AsyncProcessState.WAITING, AsyncProcessState.DONE);

    stateList.forEach(state -> {
      data1.stream().map(asyncProcessExecution -> {
        asyncProcessExecution.setId(UUID.randomUUID().toString());
        asyncProcessExecution.setState(state);
        asyncProcessExecution.setLog(
            asyncProcessExecution.getLog() + " " + asyncProcessExecution.getState().toString());
        return new ProducerRecord<>("async-process-execution",
    ...
    send(asyncProcessExecutionProducer,
        new ProducerRecord<>("async-process-execution", asyncProcessExecution.getAsyncProcessId(),
